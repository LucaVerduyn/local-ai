...
class MainWindow(QMainWindow):
    ...
    def refresh_conversations(self, query: str = "") -> None:
        ...
        if self._current_conversation_id is not None:
            self.reload_current_conversation()
        else:
            self.chat_view.show_conversation(None, [])

    def reload_current_conversation(self) -> None:
    ...
    def create_conversation(self) -> None:
        ...
        self.chat_view.set_conversations(
            self.chat.list_conversations(),
            self._current_conversation_id,
        )
        self.chat_view.show_conversation(conversation, [])
        self.navigate("chat")
        self.set_status("New chat ready")
    ...
    def send_chat_message(self, text: str) -> None:
        ...
        if conversation:
            self.chat_view.show_conversation(conversation, messages, keep_generating=False)
        self.chat_view.begin_streaming()
        self.chat_view.set_status_chip("Preparing…")
        ...
        worker.token.connect(self._on_chat_token)
    ...
    def _on_chat_token(self, token: str) -> None:
        self.chat_view.set_status_chip("Generating…")
        self.chat_view.append_token(token)

    def cancel_chat(self) -> None:
        if self._chat_worker is not None:
            self._chat_worker.request_cancel()
            self.chat_view.set_status_chip("Stopping…")
    ...
    def _on_chat_finished(self) -> None:
        ...
        # Reload only the active chat — avoid wiping composer state via full list rebuild races.
        self.reload_current_conversation()
        self.chat_view.set_conversations(
            self.chat.list_conversations(),
            self._current_conversation_id,
